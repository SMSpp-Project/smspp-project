...
static bool SolveBoth( void ) 
{
 try {
  ...
  if( hs1st ) {
    OKfo = ( ( fo1stub - fo2ndlb ) / fo1stub >= -2e-2 &&
      ( fo1stlb - fo2ndub ) / fo1stlb <= 2e-2 );
   }
     ...
     if( dis( rg ) <= 0.5 ) {  // in 50% of the cases do a ranged change
      ...
      if( tochange == 1 )
       if( dis( rg ) < 0.5 )
        ...
        static_cast< LinearFunction * >(static_cast< FRealObjective * >( TestBlock->get_objective())
          ->get_function())->modify_coefficient( strt , NC.front() );
      else
       if( dis( rg ) < 0.5 )
      ...
      if( tochange == 1 )
       if( dis( rg ) < 0.5 )
        ...
        static_cast< LinearFunction * >(static_cast< FRealObjective * >( TestBlock->get_objective())
          ->get_function())->modify_coefficient( nms.front() , NC.front() );
      else
       if( dis( rg ) < 0.5 )
