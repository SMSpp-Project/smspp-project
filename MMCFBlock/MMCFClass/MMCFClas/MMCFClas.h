...
/*--------------------------------------------------------------------------*/
  ...
  typedef double         XNumber;         ///< an "extra" variable
  typedef XNumber        *XRow;           ///< an "extra" variable array
  typedef const XNumber  cXNumber;        ///< a read-only extra variable
  typedef cXNumber       *cXRow;          ///< read-only extra variable array
    
/*--------------------------------------------------------------------------*/
    
  typedef double         XCNumber;       ///< type of arc extra cost
  typedef XCNumber       *XCRow;         ///< vector of extra costs
  typedef const XCNumber cXCNumber;      ///< a read-only extra cost
  typedef cXCNumber      *cXCRow;        ///< read-only extra cost array
...
/*--------------------------------------------------------------------------*/
/** Base class for representing the internal state of the MMCF algorithm. */
    
    class MMCFState {
    public:
        MMCFState( void ) {};
        MMCFState( const MMCFState& S ) {}
        virtual ~MMCFState() {};
    };
    
    typedef MMCFState *MMCFStatePtr;  ///< pointer to a MCFState
...
/** @name Saving/restoring the state of the solver
    ...
    virtual MMCFStatePtr MMCFGetState( void )
    /**< Save the state of the MMCFClass solver. The MMCFClass interface
     supports the notion of saving and restoring the state of the MMCFClass
     solver. The "empty" class MMCFState is defined as a place holder for state
     descriptions. MMCFGetState() creates and returns a pointer to an object of
     (a proper derived class of) class MMCFState which describes the current
     state of the MMCF solver. */
    {
        return( 0 );
    }
...
/*--------------------------------------------------------------------------*/
    ...
    virtual void MMCFPutState( MMCFStatePtr S ) { }
    
    /**< Restore the solver to the state in which it was when the state `S' was
     created with MMCFGetState() [see above]. */
...
/*--------------------------------------------------------------------------*/
   ...
   virtual void ChgXtrBnds( cXRow XLr = 0 , cXRow XUr = 0 , cIndex_Set nms = 0 ,
    ...
    /**< Changes the lower and upper range of the "extra" variables in the MMCF
     ...
     to the format of XV in GetSol() [see above]. If nms == 0, the new lower and
     upper bounds for the extra variable strt + i will be taken, respectively,
     ...
     If the vector XLr[ XUr ] is null, change just the value for  XUr[ XLr ].
     But XLr or XUr has to be defined. */
    {
        // by default there are no extra variables, hence nothing has to be done
    }
