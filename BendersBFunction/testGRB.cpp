 ...
 * GRBMILPSolver is used to solve the inner problem. The program requires as
...
#include "CWLAbstractBlockBuilderGRB.h"
...
int solve_with_BundleSolver( std::filesystem::path file_path ,
 ...
 auto inner_block_solver = new GRBMILPSolver();

 inner_block_solver->set_par( GRBMILPSolver::strOutputFile , "lp.txt" );
 ...
 inner_block_solver->set_par( inner_block_solver->dbl_par_str2idx
  ( "FeasibilityTol" ) , 1.0e-15 );

 inner_block_solver->set_par( inner_block_solver->dbl_par_str2idx
  ( "OptimalityTol" ) , 1.0e-15 );
