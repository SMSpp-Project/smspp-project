...
void MMCFCplex::SetMMCFLog( ostream *outs , const char lvl ) {
 ...
 // logfile = CPXfopen ( "cplex.log" , "w");
  int status = CPXsetlogfilename( env , "cplex.log" , "w" );
  assert( ! status );
 #endif
 }
...
MMCFClass::MMCFStatus MMCFCplex::SolveMMCF( void )
{
 ...
 // callback for taking time at different steps
 CPXsetusercutcallbackfunc(env , UserW , this );
 ...
 switch( CA ) {
  ...
  CPXsetusercutcallbackfunc(env , UserW , 0 );
...
bool MMCFCplex::GetDSol( void )
{
 ...
 {           // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  cout << " aabb = " << XtrCnst << " -- "
	   << start << " -- " << start + XtrCnst - 1 <<  endl;

  double * XtDV_ = new double[ XtrCnst ];
  CPXgetpi( env , lp , XtDV_ , start , start + XtrCnst - 1 );
  VectAssign( XtDV , XtDV_ , XtrCnst  );

  for( Index kkk = 0 ; kkk < XtrCnst ; kkk++ )
   cout << "c ( " << kkk + 1 << " ) = "<< XtDV[ kkk ] << endl;
